...
const Receipt: React.FC<ReceiptProps> = ({ order, logo, fillings }) => {
  ...
  const content = (
    <div 
      id="print-receipt" 
      className="hidden print:block bg-white text-black w-[72mm] max-w-[72mm] mx-auto text-[13px] font-mono leading-tight p-0 pb-1 box-border"
    >
      ...
      <div className="text-center mb-1">
        {receiptLogo && (
          <img 
            src={receiptLogo} 
            alt="Logo" 
            className="h-14 max-h-16 max-w-[45mm] mx-auto mb-0.5 object-contain grayscale" 
          />
        )}
        <h1 className="text-xs font-black uppercase tracking-widest leading-none mt-0.5 text-black">PASTEL DO JOEL</h1>
      ...
      <div className="border-t border-b border-dashed border-black py-1 my-0.5 text-[13px]">
        ...
        {(() => {
          ...
          return (Object.entries(groups) as [OrderType, OrderItem[]][]).map(([type, groupItems]) => (
            <div key={type} className="mb-1">
              <div className="text-center font-black uppercase text-[12px] border-b border-black py-0.5 mb-0.5 bg-black text-white">
                ...
                {groupItems.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="py-0.5 print-avoid-break">
                      ...
                      {fillingName && (
                        <div className="text-[12px] font-bold pl-3 italic text-black mt-0.5">
                          &gt; Recheio: {fillingName}
                        </div>
                      )}

                      {item.addons && item.addons.length > 0 && item.addons.map(a => (
                        <div key={a.id} className="text-[11px] font-bold pl-3 text-black">
                          + {a.name} ({a.price > 0 ? a.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : 'Grátis'})
                        </div>
                      ))}

                      {item.notes && (
                        <div className="text-[12px] font-black pl-3 text-black mt-0.5">
                          * OBS: {item.notes}
                        </div>
                      )}
                    </div>
      ...
      {/* Totais do Pedido */}
      <div className="border-t border-black pt-1 mt-0.5 space-y-0.5">
      ...
      {order.status === 'CLOSED' && (
        <div className="mt-0.5 border-b border-dashed border-black pb-1">
          ...
          {order.payments && order.payments.length > 1 ? (
            <div className="space-y-0.5">
              ...
              {order.change && order.change > 0 && (
                <div className="flex justify-between font-black text-base mt-0.5 border-t border-dotted border-black pt-0.5">
                  <span>TROCO:</span>
            ...
            <div className="space-y-0.5">
              ...
              {order.paymentAmountReceived && order.paymentAmountReceived > 0 && order.paymentMethod === 'Dinheiro' && (
                <>
                  ...
                  {order.change && order.change > 0 && (
                    <div className="flex justify-between font-black text-base border-t border-dotted border-black pt-0.5">
                      <span>TROCO:</span>
                      <span>{order.change.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                    </div>
                  )}
      ...
      <div className="text-center mt-1.5 pt-0.5">
        ...
        <p className="text-[9px] text-gray-800 mt-1">Pastelaria do Joel - Sistema de Gestão</p>
      </div>
